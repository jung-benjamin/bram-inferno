...
def model_dict():
    ...
    d = {
        'Params':
        [33.90938176807465, 8.014395700596914, 13.602488333804923, 1e-10],
        'LAMBDA': [[0.12477547120932896, 0.0], [0.0, 0.07351596086392508]],
        'alpha_': [
            9495.266495455951, 899.6204311144278, -4344.025299121975,
            9857.440586445668, 1399.307539288503, 12619.751314661358,
            -16391.488934019377, -2434.233532291585, -5138.175102070208,
            -5963.440700278297
        ],
        ...
        'y_train': [
            8038470000000000.0, 2078810000000000.0, 4963040000000000.0,
        ...
        'y_trafo':
        ['StandardNormalize', [6137578300000000.0, 3635217363511814.5]],
...
def test_load_model(tmp_path):
    ...
    p = tmp_path / 't.json'
    store_model(p)
    m1 = wastebin.WasteBin(kernels.ASQEKernelPredictor,
                           filepaths={'t': p},
                           labels=None,
                           evidence=None)
    m1.load_models(['t'])
    assert True
    m2 = wastebin.WasteBin(kernels.ASQEKernelPredictor,
                           filepaths={'t': p},
                           labels=None,
                           evidence=None)
...
def test_make_priors():
    ...
    m = wastebin.WasteBin(kernels.ASQEKernelPredictor,
                          labels=None,
                          evidence=None)
    with pm.Model():
...
def test_mixture_make_priors(tmp_path):
    ...
    with pm.Model():
        m2.load_models(['t1/t1'])
        m2._make_priors(labels=MXT_LABELS, limits=MXT_LIMITS, fallback=None)
    assert True
...
def test_mixture_model_building(tmp_path):
    ...
    with pm.Model():
        ...
        m3._make_priors(labels=MXT_LABELS, limits=MXT_LIMITS, fallback=None)
        dist = m3._make_distributions(['t1/t1', 't2/t2'], 0.1)
        m3._joint_probability([
            't1/t1',
            't2/t2',
        ], dist)
    assert True
...
def test_limits_from_file(tmp_path):
    ...
    with pm.Model():
        ...
        m2._make_priors(labels=MXT_LABELS, limits=bounds, fallback=const)
        dist = m2._make_distributions(['t1/t1', 't2/t2'], 0.1)
        m2._joint_probability([
            't1/t1',
            't2/t2',
